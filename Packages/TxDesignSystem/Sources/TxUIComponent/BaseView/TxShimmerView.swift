...
public struct TxShimmerView: ViewModifier {
    public enum Mode {
    ...
    public static let defaultGradient = Gradient(colors: [
        .black.opacity(0.2), // translucent
        .black.opacity(0.7), // opaque
...
public extension View {
    ...
    @ViewBuilder func shimmering(
        active: Bool = true,
        animation: Animation = TxShimmerView.defaultAnimation,
        gradient: Gradient = TxShimmerView.defaultGradient,
        bandSize: CGFloat = 0.3,
        mode: TxShimmerView.Mode = .mask
    ) -> some View {
        if active {
            modifier(TxShimmerView(animation: animation,
                                   gradient: gradient,
                                   bandSize: bandSize,
                                   mode: mode))
        } else {
